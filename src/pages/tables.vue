...
async function loadSchema() {
  ...
  try {
    ...
    selectedTableName.value = selectedTableName.value || schema.value.tables[0]?.name || "";
...
async function saveRow() {
  ...
  try {
    if (originalRow.value) {
      await updateRow(connectionId.value, selectedTableName.value, originalRow.value, parsed);
...
function updateCell(row: Record<string, unknown>, columnName: string, value: string) {
...
<template>
  <div class="flex h-full min-h-0 flex-col gap-4">
    ...
    <div class="grid min-h-0 flex-1 gap-4 xl:grid-rows-[minmax(0,1fr)_minmax(180px,28%)]">
      <div class="grid min-h-0 gap-4 xl:grid-cols-[320px_1fr]">
      <aside class="flex min-h-0 flex-col rounded-lg border border-default bg-default/30 p-4">
        ...
        <div class="min-h-0 flex-1 space-y-2 overflow-auto pr-1">
          ...
          >
            <div class="flex items-center justify-between gap-3">
              <span class="truncate text-sm font-medium text-highlighted">{{ table.name }}</span>
              <UBadge size="sm" color="neutral" variant="subtle">{{ table.access }}</UBadge>
            </div>
            <p class="mt-1 truncate text-xs text-muted">
              {{ table.columns.length }} columns
              <span v-if="table.primaryKey.length"> - PK {{ table.primaryKey.join(", ") }}</span>
            </p>
          </button>
        </div>
      </aside>

      <section class="min-h-0 min-w-0 flex flex-col rounded-lg border border-default bg-default/30">
        <div class="shrink-0 flex flex-wrap items-center justify-between gap-3 border-b border-default p-4">
          <div class="min-w-0">
            <h2 class="truncate text-base font-semibold text-highlighted">
              {{ selectedTableName || "No table selected" }}
            </h2>
            <p class="mt-1 text-xs text-muted">
              {{ tablePage?.total == null ? "Row count unknown" : `${tablePage.total} rows` }} - page {{ page + 1 }}
            </p>
          </div>
        ...
        <div class="min-h-0 flex-1 overflow-auto">
          <table class="min-w-full text-sm">
            ...
            <tbody>
              <tr v-if="loadingRows">
                <td class="px-3 py-6 text-muted" :colspan="(tablePage?.columns.length ?? 0) + 1">
                  Loading rows...
                </td>
              </tr>
              <tr v-for="(row, rowIndex) in tablePage?.rows ?? []" :key="rowIndex" class="border-b border-default/60">
                <td
                  v-for="column in tablePage?.columns ?? []"
                  :key="column.name"
                ...
                >
                  <input
                    ...
                    @change="updateCell(row, column.name, ($event.target as HTMLInputElement).value)"
                  />
                </td>
                <td
                ...
                >
                  <UButton icon="i-lucide-save" size="xs" variant="ghost" aria-label="Save row" @click="editRow(row); saveRow()" />
                  <UButton icon="i-lucide-file-pen-line" size="xs" color="neutral" variant="ghost" aria-label="Edit JSON" @click="editRow(row)" />
                  <UButton icon="i-lucide-trash-2" size="xs" color="error" variant="ghost" aria-label="Delete row" @click="deleteRow(row)" />
                </td>
              </tr>
            </tbody>
          </table>
      ...
      <section class="flex min-h-0 flex-col rounded-lg border border-default bg-default/30 p-4">
        <div class="mb-3 shrink-0 flex items-center justify-between gap-3">
          <h2 class="text-base font-semibold text-highlighted">Raw SQL</h2>
          <UButton icon="i-lucide-play" :loading="runningSql" :disabled="!sql.trim()" @click="runSql">
            Run
          </UButton>
        </div>
        <textarea
          ...
          class="min-h-0 flex-1 resize-none overflow-auto rounded-md border border-default bg-default px-3 py-2 font-mono text-xs text-highlighted outline-none focus:border-primary"
        />
        <pre class="mt-3 min-h-20 max-h-32 shrink-0 overflow-auto rounded-md border border-default bg-black p-3 text-xs text-neutral-100">{{ sqlResult || "SQL results will appear here." }}</pre>
      </section>
    </div>
    ...
    <USlideover
      ...
      side="right"
      :ui="{ content: 'sm:max-w-2xl', body: 'min-h-0 flex-1', footer: 'shrink-0' }"
    >
      <template #body>
        <textarea
          v-model="rowJson"
